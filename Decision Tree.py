# Predicting Dielectric loss (D) and Dielectric constant (k) of HDPE TiO2 using Decision Tree .Here, Frequency and HDPE% is feature variables and  D and K is target variables.
...
# Train a DecisionTreeRegressor model for 'D'
decision_tree_model_D = DecisionTreeRegressor(random_state=42)
decision_tree_model_D.fit(X_train, y_train['D'])  # Train for 'D'

# Train a DecisionTreeRegressor model for 'K'
decision_tree_model_K = DecisionTreeRegressor(random_state=42)
decision_tree_model_K.fit(X_train, y_train['K'])  # Train for 'K'

# Evaluate the model for 'D'
y_pred_D = decision_tree_model_D.predict(X_test)
...
y_pred_K = decision_tree_model_K.predict(X_test)
...
y1_30=decision_tree_model_K.predict(df_30[["Fre (Hz)","HDPE%"]])
np.savetxt('dt_30_K.csv', y1_30, delimiter=',')

df_30=pd.read_excel("C:/Users/USER/TAREQ/H30T.xlsx")
y_30=decision_tree_model_D.predict(df_30[["Fre (Hz)","HDPE%"]])
np.savetxt('dt_30_D.csv', y_30, delimiter=',')




# Show the relationship between Fre(Hz), HDPE%, D, and K using a pairplot
data['D Predicted'] = decision_tree_model_D.predict(X)  # Predictions for 'D'
data['K Predicted'] = decision_tree_model_K.predict(X)  # Predictions for 'K'
...
cross_val_scores = cross_val_score(decision_tree_model_D, X, y['D'], cv=5, scoring='neg_mean_squared_error')
mean_cv_mse = -np.mean(cross_val_scores)
print(f"Mean Cross-Validation MSE for 'D' model: {mean_cv_mse}")

cross_val_scores = cross_val_score(decision_tree_model_K, X, y['K'], cv=5, scoring='neg_mean_squared_error')
mean_cv_mse = -np.mean(cross_val_scores)
...
while True:
    try:
        ...
        prediction_D = decision_tree_model_D.predict(user_data)
        prediction_K = decision_tree_model_K.predict(user_data)
