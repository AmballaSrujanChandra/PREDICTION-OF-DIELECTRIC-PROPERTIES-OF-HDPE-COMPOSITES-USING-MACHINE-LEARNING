# Predicting Dielectric loss (D) and Dielectric constant (k) of HDPE TiO2 using adaBoost Algorithm.Here, Frequency and HDPE% is feature variables and  D and K is target variables.
...
# Print the columns and their values after removing
print(data)
...
# Train an AdaBoostRegressor model for 'D'
adaboost_model_D = AdaBoostRegressor(n_estimators=100, random_state=42)
adaboost_model_D.fit(X_train, y_train['D'])  # Train for 'D'

# Train an AdaBoostRegressor model for 'K'
adaboost_model_K = AdaBoostRegressor(n_estimators=100, random_state=42)
adaboost_model_K.fit(X_train, y_train['K'])  # Train for 'K'


# Evaluate the model for 'D'
y_pred_D = adaboost_model_D.predict(X_test)
...
y_pred_K = adaboost_model_K.predict(X_test)
...
y1_30=adaboost_model_K.predict(df_30[["Fre (Hz)","HDPE%"]])
np.savetxt('ab_30_K.csv', y1_30, delimiter=',')

df_30=pd.read_excel("C:/Users/USER/TAREQ/H30T.xlsx")
y_30=adaboost_model_D.predict(df_30[["Fre (Hz)","HDPE%"]])
np.savetxt('ab_30_D.csv', y_30, delimiter=',')

# Show the relationship between Fre(Hz), HDPE%, D, and K using a pairplot
data['D Predicted'] = adaboost_model_D.predict(X)  # Predictions for 'D'
data['K Predicted'] = adaboost_model_K.predict(X)  # Predictions for 'K'
...
cross_val_scores = cross_val_score(adaboost_model_D, X, y['D'], cv=5, scoring='neg_mean_squared_error')
mean_cv_mse = -np.mean(cross_val_scores)
print(f"Mean Cross-Validation MSE for 'D' model: {mean_cv_mse}")

cross_val_scores = cross_val_score(adaboost_model_K, X, y['K'], cv=5, scoring='neg_mean_squared_error')
mean_cv_mse = -np.mean(cross_val_scores)
print(f"Mean Cross-Validation MSE for 'K' model: {mean_cv_mse}")

- The Mean Cross-Validation MSE for the 'D' model is approximately 3.38e-05, which is a very low value. 
    This suggests that the model is performing very well in predicting the 'D' (Dielectric Loss) values.

- The Mean Cross-Validation MSE for the 'K' model is approximately 0.588, which is a bit higher.
    While it's not as low as the 'D' MSE, it's still reasonable. 
    However, it depends on the scale and nature of your data. I
    f 'K' values have a large range, this MSE might be acceptable.
...
while True:
    try:
        ...
        prediction_D = adaboost_model_D.predict(user_data)
        prediction_K = adaboost_model_K.predict(user_data)
