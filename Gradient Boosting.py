# Predicting Dielectric loss (D) and Dielectric constant (k) of HDPE TiO2 using Gradient Boosting Regressor (Ensemble Method).Here, Frequency and HDPE% is feature variables and  D and K is target variables.
...
# Train a GradientBoostingRegressor model for 'D'
gb_model_D = GradientBoostingRegressor(n_estimators=100, random_state=42)
gb_model_D.fit(X_train, y_train['D'])  # Train for 'D'

# Train a GradientBoostingRegressor model for 'K'
gb_model_K = GradientBoostingRegressor(n_estimators=100, random_state=42)
gb_model_K.fit(X_train, y_train['K'])  # Train for 'K'

# Evaluate the model for 'D'
y_pred_D = gb_model_D.predict(X_test)
...
y_30=gb_model_D.predict(df_30[["Fre (Hz)","HDPE%"]])
np.savetxt('gb_30_D.csv', y_30, delimiter=',')

y1_30=gb_model_K.predict(df_30[["Fre (Hz)","HDPE%"]])
np.savetxt('gb_30_K.csv', y1_30, delimiter=',')

# Evaluate the model for 'K'
y_pred_K = gb_model_K.predict(X_test)
...
data['D Predicted'] = gb_model_D.predict(X)  # Predictions for 'D'
data['K Predicted'] = gb_model_K.predict(X)  # Predictions for 'K'
...
cross_val_scores = cross_val_score(gb_model_D, X, y['D'], cv=5, scoring='neg_mean_squared_error')
mean_cv_mse = -np.mean(cross_val_scores)
print(f"Mean Cross-Validation MSE for 'D' model: {mean_cv_mse}")

cross_val_scores = cross_val_score(gb_model_K, X, y['K'], cv=5, scoring='neg_mean_squared_error')
mean_cv_mse = -np.mean(cross_val_scores)
...
while True:
    try:
        ...
        prediction_D = gb_model_D.predict(user_data)
        prediction_K = gb_model_K.predict(user_data)
